...
const ThreeMath = THREE.MathUtils || THREE.Math;
...
class World extends Animation {
  ...
  resize() {
    ...
    const fovRad = this.fov * ThreeMath.DEG2RAD;
  ...
  rotateLayer( rotation, scramble, callback ) {
    ...
    this.rotationTween = new Tween( {
      ...
      onComplete: () => {
        ...
        this.game.cube.object.rotation.setFromVector3( this.snapRotation( new THREE.Vector3( this.game.cube.object.rotation.x, this.game.cube.object.rotation.y, this.game.cube.object.rotation.z ) ) );
        this.group.rotation.setFromVector3( this.snapRotation( new THREE.Vector3( this.group.rotation.x, this.group.rotation.y, this.group.rotation.z ) ) );
  ...
  rotateCube( rotation, callback ) {
    ...
    this.rotationTween = new Tween( {
      ...
      onComplete: () => {
        ...
        this.edges.rotation.setFromVector3( this.snapRotation( new THREE.Vector3( this.edges.rotation.x, this.edges.rotation.y, this.edges.rotation.z ) ) );
        this.game.cube.object.rotation.copy( this.edges.rotation );
        callback();
...
class ConfettiStage extends Animation {
  ...
  resizeViewport() {
    ...
    const fovRad = this.game.world.camera.fov * ThreeMath.DEG2RAD;
...
class Particle {
  ...
  constructor( confetti ) {
    ...
    this.size = ThreeMath.randFloat( this.options.size.min, this.options.size.max );
  ...
  reset( randomHeight = true ) {
    ...
    this.speed = ThreeMath.randFloat( this.options.speed.min, this.options.speed.max ) * - 1;
    this.mesh.position.x = ThreeMath.randFloat( - this.confetti.width / 2, this.confetti.width / 2 );
    this.mesh.position.y = ( randomHeight )
      ? ThreeMath.randFloat( this.size, this.confetti.height + this.size )
      : this.size;

    this.revolutionSpeed = ThreeMath.randFloat( this.options.revolution.min, this.options.revolution.max );
...
class Storage {
  ...
  saveGame() {
    ...
    this.game.cube.pieces.forEach( piece => {
      ...
      gameCubeData.rotations.push( new THREE.Vector3( piece.rotation.x, piece.rotation.y, piece.rotation.z ) );
...
export function initializeRubikGame() {
...
/**
 * Returns the active Game instance, or null if not yet initialised.
 */
export function getGame() {
  return gameInstance;
}

// ─── Layer / face constants usable by callers ───────────────────────────────
...
export const FACE_MOVES = {
  // Standard Rubik's notation → { axis, row, angle }
  U:  { axis: 'y', row:  1, angle:  Math.PI / 2 },
  "U'": { axis: 'y', row:  1, angle: -Math.PI / 2 },
  D:  { axis: 'y', row: -1, angle: -Math.PI / 2 },
  "D'": { axis: 'y', row: -1, angle:  Math.PI / 2 },
  R:  { axis: 'x', row:  1, angle: -Math.PI / 2 },
  "R'": { axis: 'x', row:  1, angle:  Math.PI / 2 },
  L:  { axis: 'x', row: -1, angle:  Math.PI / 2 },
  "L'": { axis: 'x', row: -1, angle: -Math.PI / 2 },
  F:  { axis: 'z', row:  1, angle: -Math.PI / 2 },
  "F'": { axis: 'z', row:  1, angle:  Math.PI / 2 },
  B:  { axis: 'z', row: -1, angle:  Math.PI / 2 },
  "B'": { axis: 'z', row: -1, angle: -Math.PI / 2 },
};

/**
 * Rotate a single cube layer using standard Rubik's notation.
 *
 * @param {string} notation  e.g. "R", "U'", "F"
 * @param {Function} [callback]  called when the animation completes
 */
export function rotateFace( notation, callback = () => {} ) {
  const game = getGame();
  if ( ! game ) return;

  const def = FACE_MOVES[ notation ];
  if ( ! def ) { console.warn( `rotateFace: unknown notation "${notation}"` ); return; }

  const position = new THREE.Vector3();
  position[ def.axis ] = def.row;

  game.controls.keyboardMove( LAYER_MOVE, { axis: def.axis, angle: def.angle, position }, callback );
}

/**
 * Rotate the whole cube on a world axis.
 *
 * @param {'x'|'y'|'z'} axis
 * @param {number} angle  radians — use Math.PI/2 for 90 °
 * @param {Function} [callback]
 */
export function rotateCube( axis, angle, callback = () => {} ) {
  const game = getGame();
  if ( ! game ) return;

  game.controls.keyboardMove( CUBE_MOVE, { axis, angle }, callback );
}

/**
 * Programmatically start a new game (triggers scramble + zoom transition).
 */
export function startGame() {
  const game = getGame();
  if ( game ) game.game( SHOW );
}

/**
 * Return to the menu from play state.
 */
export function exitGame() {
  const game = getGame();
  if ( game ) game.game( HIDE );
}

/**
 * Apply a full algorithm string, e.g. "R U R' U'".
 * Moves are executed sequentially, each waits for the previous animation.
 *
 * @param {string} algorithm  space-separated standard notation
 * @param {Function} [onComplete]  called when all moves finish
 */
export function applyAlgorithm( algorithm, onComplete = () => {} ) {
  const moves = algorithm.trim().split( /\s+/ );

  const next = () => {
    if ( moves.length === 0 ) { onComplete(); return; }
    rotateFace( moves.shift(), next );
  };
